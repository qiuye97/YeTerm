...
<div align="center">
<img src="assets/screenshots/crt_live.gif" width="620" alt="CRT 终端演示：绿色磷光文字在扫描线下逐字吐出、ASCII banner、中日韩混排，随后换台切到琥珀色主题、消磁波纹，最后是彩色显像管下的 ANSI 颜色">
<br><em>这段是 app 自己录的（<code>scripts/make_demo_gif.sh</code>）。输出限速在 4800 波特，所以文字像当年的串口终端那样一个个爬上屏幕——底下是活的扫描线、辉光、闪烁和雪花。然后换台切主题（老电视换台那一闪）、消磁抖一圈，最后一套预设是彩色显像管。</em>
</div>
...
- **背景图片** —— 两种模式都能铺。CRT 模式下它替换的是磷光背后那层「没被电子束打亮的屏幕」，所以发光的文字始终保持自己的颜色，不会被亮图洗掉；图跟着屏幕弧度一起鼓、吃扫描线。还可以选择让整张图被磷光染色，像这台老显示器正在显示它。
